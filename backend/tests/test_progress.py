...
Both endpoints sit behind the player's own session token (PROJECT_PLAN
§2.1), so every request here carries the headers `_play` hands back.
"""
...
def _play(client, db_session, username, math_type, grade, difficulty, correct_count, time_used=60):
    """Run one quiz for `username`, answering `correct_count` right.

    Returns that player's auth headers, so a test can go straight from
    playing to reading their own stats.
    """
    r = client.post("/api/users", json={"username": username, "pin": "1234"})
    if r.status_code == 409:  # already signed up earlier in this test
        r = client.post("/api/users/login", json={"username": username, "pin": "1234"})
...
def test_stats_empty_for_new_player(client, signup):
    auth = signup(client, "NoQuizzes")
    r = client.get("/api/users/NoQuizzes/stats", headers=auth)
...
def test_stats_aggregate_across_quizzes(client, db_session):
    ...
    auth = _play(client, db_session, "Ada", "geometry", "2", "easy", correct_count=8)

    body = client.get("/api/users/Ada/stats", headers=auth).json()
...
def test_stats_recent_is_capped_at_five_and_newest_first(client, db_session):
    for i in range(7):
        auth = _play(
            client, db_session, "Bob", "addition", "1", "easy", correct_count=i, time_used=10 + i
        )
    body = client.get("/api/users/Bob/stats", headers=auth).json()
    assert body["totalQuizzes"] == 7
    assert len(body["recent"]) == 5


def test_stats_username_is_case_insensitive(client, db_session):
    auth = _play(client, db_session, "Cara", "addition", "1", "easy", correct_count=5)
    assert client.get("/api/users/CARA/stats", headers=auth).json()["totalQuizzes"] == 1


def test_suggested_level_none_without_history(client, signup):
    auth = signup(client, "Nobody")
    r = client.get("/api/users/Nobody/suggested-level", headers=auth)
    assert r.status_code == 200
...
def test_suggested_level_bumps_difficulty_after_high_scores(client, db_session):
    ...
    auth = _play(client, db_session, "Deb", "addition", "3", "easy", correct_count=9)
    body = client.get("/api/users/Deb/suggested-level", headers=auth).json()
    assert body["grade"] == "3"
    assert body["difficulty"] == "medium"


def test_suggested_level_eases_down_after_low_scores(client, db_session):
    auth = _play(client, db_session, "Eli", "addition", "4", "hard", correct_count=2)
    body = client.get("/api/users/Eli/suggested-level", headers=auth).json()
    assert body["grade"] == "4"
    assert body["difficulty"] == "medium"


def test_suggested_level_holds_for_middling_scores(client, db_session):
    auth = _play(client, db_session, "Fay", "addition", "2", "medium", correct_count=6)
    body = client.get("/api/users/Fay/suggested-level", headers=auth).json()
    assert body["grade"] == "2"
    assert body["difficulty"] == "medium"
...
def test_suggested_level_is_per_topic(client, db_session):
    ...
    auth = _play(client, db_session, "Gil", "addition", "3", "easy", correct_count=10)

    add = client.get(
        "/api/users/Gil/suggested-level", params={"mathType": "addition"}, headers=auth
    ).json()
    assert (add["grade"], add["difficulty"]) == ("3", "medium")  # aced easy → up
    assert add["mathType"] == "addition"

    frac = client.get(
        "/api/users/Gil/suggested-level", params={"mathType": "fractions"}, headers=auth
    ).json()
...
def test_suggested_level_fresh_topic_starts_easy_at_home_grade(client, db_session):
    ...
    auth = _play(client, db_session, "Hana", "addition", "4", "hard", correct_count=10)

    r = client.get(
        "/api/users/Hana/suggested-level", params={"mathType": "geometry"}, headers=auth
    ).json()
...
def test_suggested_level_fresh_topic_respects_topic_entry_grade(client, db_session):
    ...
    auth = _play(client, db_session, "Ivo", "addition", "K", "easy", correct_count=8)

    r = client.get(
        "/api/users/Ivo/suggested-level", params={"mathType": "percentages"}, headers=auth
    ).json()
    assert (r["grade"], r["difficulty"]) == ("4", "easy")
...
def test_suggested_level_never_steps_below_topic_entry_grade(client, db_session):
    ...
    auth = _play(client, db_session, "Jo", "fractions", "2", "easy", correct_count=1)

    r = client.get(
        "/api/users/Jo/suggested-level", params={"mathType": "fractions"}, headers=auth
    ).json()
...
def test_suggested_level_without_topic_is_overall(client, db_session):
    auth = _play(client, db_session, "Kai", "addition", "2", "easy", correct_count=10)
    r = client.get("/api/users/Kai/suggested-level", headers=auth).json()
